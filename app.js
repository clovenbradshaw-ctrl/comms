...
Current Key: ${CryptoManager.getCurrentKey() ? 'Loaded ✓' : 'Not set ✗'}</pre>
    ...
    class WorkspaceStore {
      constructor(storageKey = 'secureChat.workspaces') {
      ...
      getStorage() {
        ...
        try {
          const testKey = '__workspace_test__';
          window.localStorage.setItem(testKey, '1');
          window.localStorage.removeItem(testKey);
          return window.localStorage;
        } catch (error) {
          console.warn('Workspace storage unavailable.', error);
          return null;
        }
      }
      ...
      load() {
        ...
        try {
          ...
          console.warn('Failed to parse workspace store.', error);
          return [];
        }
      }
      ...
      save() {
        ...
        try {
          this.storage.setItem(this.storageKey, JSON.stringify(this.workspaces));
        } catch (error) {
          console.warn('Failed to persist workspace store.', error);
        }
      }
      ...
      createWorkspace({ id, name, description = '', settings }) {
        const workspace = {
          id,
          name,
          description,
          createdAt: Date.now(),
          settings: {
            access: settings?.access || 'invite',
            approvalCount: settings?.approvalCount || 1,
            allowedDomains: Array.isArray(settings?.allowedDomains)
              ? settings.allowedDomains.filter(Boolean)
              : []
          },
          members: [
            {
      ...
      updateWorkspace(id, mutator) {
        const workspace = this.getById(id);
        if (!workspace || typeof mutator !== 'function') {
          return null;
        }
        mutator(workspace);
        this.save();
      ...
      addPendingRequest(workspaceId, request) {
        const workspace = this.getById(workspaceId);
        if (!workspace) {
          return null;
        }
        workspace.pendingRequests.push({
          ...request,
          id: request.id || this.generateId('request'),
          createdAt: Date.now(),
          approvals: [],
          rejections: [],
          status: 'pending'
        ...
        this.save();
        return workspace.pendingRequests[workspace.pendingRequests.length - 1];
      }

      approveRequest(workspaceId, requestId) {
        return this.updateWorkspace(workspaceId, (workspace) => {
          ...
          workspace.pendingRequests = workspace.pendingRequests.filter((item) => item.id !== requestId);
        });
      }

      rejectRequest(workspaceId, requestId) {
        return this.updateWorkspace(workspaceId, (workspace) => {
          const request = workspace.pendingRequests.find((item) => item.id === requestId);
          if (!request) {
            return;
    ...
    class WorkspaceHub {
      ...
      toggleCreateForm(force) {
        this.state.showCreateForm = typeof force === 'boolean'
          ? force
      ...
      setCreateFeedback(message) {
        this.state.createFeedback = message || '';
        this.render();
      }

      setJoinFeedback(message) {
        this.state.joinFeedback = message || '';
        this.render();
      }
      ...
      render() {
        ...
        this.container.innerHTML = `
          <div class="workspace-hub-card">
            <div class="workspace-hub-header">
              <div class="workspace-hub-title">
                <span class="workspace-hub-icon">🗂️</span>
                <div>
                  <div class="workspace-hub-label">Persistent Workspaces</div>
                  <div class="workspace-hub-subtitle">Create or join secure spaces for your teams</div>
                </div>
              </div>
              <button type="button" class="workspace-hub-toggle" data-action="toggle-create">
                ${this.state.showCreateForm ? 'Close' : 'New Workspace'}
              </button>
            </div>

            ${this.state.showCreateForm ? this.renderCreateForm(origin) : ''}

            ${!this.state.showCreateForm && this.state.createFeedback
              ? `<div class="workspace-feedback" role="status">${escapeHTML(this.state.createFeedback)}</div>`
              : ''}

            ${hasWorkspaces ? this.renderWorkspaceList(workspaces, origin) : this.renderEmptyState()}

            ${this.renderJoinForm(workspaces, origin)}
          </div>
        `;
      ...
      renderEmptyState() {
        return `
          <div class="workspace-empty">
            <p>No workspaces yet. Create one to share a permanent link with your team.</p>
          </div>
        `;
      }

      renderCreateForm(origin = '') {
        const feedback = this.state.createFeedback
          ? `<div class="workspace-feedback" role="status">${escapeHTML(this.state.createFeedback)}</div>`
          : '';
        return `
          <form class="workspace-create" data-action="create-workspace">
            <div class="workspace-field">
              <label for="workspaceName">Workspace Name</label>
              <input id="workspaceName" name="name" type="text" placeholder="e.g. Product Team" maxlength="50" required>
            </div>
            <div class="workspace-field">
              <label for="workspaceId">Workspace URL</label>
              <div class="workspace-url">
                <span class="workspace-url-base">${escapeHTML(origin)}/#/</span>
                <input id="workspaceId" name="id" type="text" placeholder="team-hub" pattern="[a-z0-9-]+" maxlength="30" required>
              </div>
            </div>
            <div class="workspace-field">
              <label for="workspaceDesc">Description <span class="muted">(optional)</span></label>
              <textarea id="workspaceDesc" name="description" rows="2" placeholder="What is this workspace for?"></textarea>
            </div>
            <div class="workspace-field">
              <label>Access</label>
              <div class="workspace-radio-group">
                <label><input type="radio" name="access" value="invite" checked> Invite only</label>
                <label><input type="radio" name="access" value="approval"> Request to join</label>
                <label><input type="radio" name="access" value="domain"> Domain allow list</label>
              </div>
            </div>
            <div class="workspace-field" data-domain-input hidden>
              <label for="workspaceDomains">Allowed domains <span class="muted">(comma separated)</span></label>
              <input id="workspaceDomains" name="domains" type="text" placeholder="@company.com, @partners.io">
            </div>
            <div class="workspace-field" data-approval-input hidden>
              <label for="workspaceApprovals">Approvals required</label>
              <select id="workspaceApprovals" name="approvals">
      ...
      renderWorkspaceList(workspaces, origin = '') {
        const list = workspaces
          .map((workspace) => {
            const pendingCount = workspace.pendingRequests?.length || 0;
            const pendingLabel = pendingCount > 0
              ? `<span class="workspace-pill">${pendingCount} pending</span>`
              : '';
            const accessLabel = workspace.settings?.access === 'approval'
              ? 'Request to join'
              : workspace.settings?.access === 'domain'
                ? 'Domain restricted'
                : 'Invite only';
            const description = workspace.description
              ? `<p class="workspace-desc">${escapeHTML(workspace.description)}</p>`
              : '';
            const requests = pendingCount > 0
              ? `
                <div class="workspace-requests" data-workspace="${escapeHTML(workspace.id)}">
                  ${workspace.pendingRequests.map((request) => this.renderPendingRequest(workspace, request)).join('')}
                </div>
              `
              : '';

            return `
              <article class="workspace-item" data-id="${escapeHTML(workspace.id)}">
                <header class="workspace-item-header">
                  <div>
                    <h3>${escapeHTML(workspace.name)}</h3>
                    <div class="workspace-meta">
                      <span class="workspace-slug">#/${escapeHTML(workspace.id)}</span>
                      <span>${escapeHTML(accessLabel)}</span>
                      ${pendingLabel}
                    </div>
                  </div>
                  <div class="workspace-item-actions">
                    <button type="button" class="btn btn-secondary" data-action="open" data-id="${escapeHTML(workspace.id)}">Open</button>
                    <button type="button" class="btn btn-secondary" data-action="copy-link" data-id="${escapeHTML(workspace.id)}" data-origin="${escapeHTML(origin)}">Copy Link</button>
                  </div>
                </header>
                ${description}
                ${requests}
        ...
        return `<div class="workspace-list">${list}</div>`;
      }

      renderPendingRequest(workspace, request) {
        return `
          <div class="workspace-request" data-request-id="${escapeHTML(request.id)}">
            <div>
              <strong>${escapeHTML(request.requester?.name || 'Anonymous')}</strong>
              ${request.requester?.email ? `<span class="workspace-request-email">${escapeHTML(request.requester.email)}</span>` : ''}
              <div class="workspace-request-time">Requested ${this.relativeTime(request.createdAt)}</div>
              ${request.requester?.message ? `<p class="workspace-request-message">${escapeHTML(request.requester.message)}</p>` : ''}
            </div>
            <div class="workspace-request-actions">
              <button type="button" class="btn btn-primary" data-action="approve-request" data-id="${escapeHTML(workspace.id)}" data-request="${escapeHTML(request.id)}">Approve</button>
              <button type="button" class="btn btn-secondary" data-action="reject-request" data-id="${escapeHTML(workspace.id)}" data-request="${escapeHTML(request.id)}">Reject</button>
            </div>
          </div>
        `;
      }

      renderJoinForm(workspaces, origin = '') {
        const feedback = this.state.joinFeedback
          ? `<div class="workspace-feedback" role="status">${escapeHTML(this.state.joinFeedback)}</div>`
          : '';
        const placeholderId = workspaces[0]?.id
          ? `#/${workspaces[0].id}`
          : (origin ? `${origin}/#/workspace-id` : 'workspace-id');
        return `
          <form class="workspace-join" data-action="join-workspace">
            <h4>Request access</h4>
            <div class="workspace-field">
              <label for="joinWorkspaceId">Workspace URL or ID</label>
              <input id="joinWorkspaceId" name="workspace" type="text" placeholder="${escapeHTML(placeholderId)}" required>
            </div>
            <div class="workspace-field">
              <label for="joinName">Your name</label>
              <input id="joinName" name="name" type="text" placeholder="Ada Lovelace" required>
            </div>
            <div class="workspace-field">
              <label for="joinEmail">Email <span class="muted">(optional)</span></label>
              <input id="joinEmail" name="email" type="email" placeholder="you@example.com">
            </div>
            <div class="workspace-field">
              <label for="joinMessage">Message <span class="muted">(optional)</span></label>
              <textarea id="joinMessage" name="message" rows="2" placeholder="Tell admins why you want to join"></textarea>
            </div>
            ${feedback}
            <div class="workspace-actions">
      ...
      bindEvents() {
        const toggle = this.container.querySelector('[data-action="toggle-create"]');
        if (toggle) {
          toggle.addEventListener('click', () => this.toggleCreateForm());
        }

        const createForm = this.container.querySelector('[data-action="create-workspace"]');
        if (createForm) {
          const nameInput = createForm.querySelector('input[name="name"]');
          const idInput = createForm.querySelector('input[name="id"]');
          const accessInputs = createForm.querySelectorAll('input[name="access"]');
          const domainField = createForm.querySelector('[data-domain-input]');
          const approvalField = createForm.querySelector('[data-approval-input]');
          ...
          if (nameInput && idInput) {
            nameInput.addEventListener('input', () => {
              ...
              idInput.value = this.slugify(nameInput.value);
            });
            idInput.addEventListener('input', () => {
              this.state.slugLocked = idInput.value.trim().length > 0;
            });
          }
          ...
          accessInputs.forEach((input) => {
            input.addEventListener('change', () => {
              if (input.value === 'domain') {
                domainField?.removeAttribute('hidden');
                approvalField?.setAttribute('hidden', '');
              } else if (input.value === 'approval') {
                approvalField?.removeAttribute('hidden');
                domainField?.setAttribute('hidden', '');
              } else {
                domainField?.setAttribute('hidden', '');
                approvalField?.setAttribute('hidden', '');
              }
          ...
          createForm.addEventListener('submit', (event) => {
            event.preventDefault();
            this.handleCreateSubmit(new FormData(createForm));
          });
        }

        const joinForm = this.container.querySelector('[data-action="join-workspace"]');
        if (joinForm) {
          joinForm.addEventListener('submit', (event) => {
            event.preventDefault();
            this.handleJoinSubmit(new FormData(joinForm));
          });
        }

        this.container.querySelectorAll('[data-action="open"]').forEach((button) => {
        ...
        this.container.querySelectorAll('[data-action="copy-link"]').forEach((button) => {
          button.addEventListener('click', async () => {
            const id = button.dataset.id;
            const origin = button.dataset.origin || (typeof window !== 'undefined' ? window.location.origin : '');
            const link = `${origin}/#/${id}`;
            try {
              if (typeof navigator !== 'undefined' && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
                await navigator.clipboard.writeText(link);
                this.setCreateFeedback('Invite link copied to clipboard.');
              } else {
                throw new Error('Clipboard API unavailable');
              }
            } catch (error) {
              console.warn('Failed to copy workspace link.', error);
              this.setCreateFeedback('Copy this link manually: ' + link);
            }
          });
        });

        this.container.querySelectorAll('[data-action="approve-request"]').forEach((button) => {
          button.addEventListener('click', () => {
            const workspaceId = button.dataset.id;
            const requestId = button.dataset.request;
            this.store.approveRequest(workspaceId, requestId);
            this.setCreateFeedback('Request approved.');
          });
        });

        this.container.querySelectorAll('[data-action="reject-request"]').forEach((button) => {
          button.addEventListener('click', () => {
            const workspaceId = button.dataset.id;
            const requestId = button.dataset.request;
            this.store.rejectRequest(workspaceId, requestId);
            this.setCreateFeedback('Request rejected.');
          });
        });
      }
      ...
      handleCreateSubmit(formData) {
        ...
        const approvals = Number.parseInt((formData.get('approvals') || '1').toString(), 10) || 1;
        const domainsRaw = (formData.get('domains') || '').toString();
        ...
        if (!name || !id) {
          this.setCreateFeedback('Name and workspace URL are required.');
          return;
        }

        id = this.slugify(id);
        if (!id) {
          this.setCreateFeedback('Workspace URL must include letters, numbers, or hyphens.');
          return;
        }

        if (this.store.getById(id)) {
          this.setCreateFeedback('That workspace URL is already in use. Choose another.');
          return;
        }
        ...
        this.state.slugLocked = false;
        this.toggleCreateForm(false);
        this.setCreateFeedback(`Workspace “${name}” created. Share the invite link to get started.`);
      }

      handleJoinSubmit(formData) {
        const workspaceInput = (formData.get('workspace') || '').toString().trim();
        const name = (formData.get('name') || '').toString().trim();
        const email = (formData.get('email') || '').toString().trim();
        const message = (formData.get('message') || '').toString().trim();

        if (!workspaceInput || !name) {
          this.setJoinFeedback('Workspace and name are required.');
          return;
        }

        const workspaceId = this.extractWorkspaceId(workspaceInput);
        const workspace = this.store.getById(workspaceId);
        if (!workspace) {
          this.setJoinFeedback('Workspace not found. Check the link or ask for a new invite.');
          return;
        }

        if (workspace.settings?.access === 'invite') {
          this.setJoinFeedback('This workspace requires an invite link from a member.');
          return;
        }

        if (workspace.settings?.access === 'domain') {
          const domain = email.split('@')[1] ? `@${email.split('@')[1]}` : '';
          const allowed = workspace.settings.allowedDomains || [];
          if (domain && allowed.some((entry) => entry.toLowerCase() === domain.toLowerCase())) {
            this.store.updateWorkspace(workspaceId, (item) => {
              item.members.push({
                id: this.store.generateId('member'),
                displayName: name,
                email,
                role: 'member',
                joinedAt: Date.now()
            ...
            this.setJoinFeedback('Joined automatically. You now have access.');
            return;
          }
        }

        this.store.addPendingRequest(workspaceId, {
          requester: { name, email, message }
        });
        this.setJoinFeedback('Request submitted. An admin will review it soon.');
      }
      ...
      extractWorkspaceId(value) {
        ...
        if (hashIndex !== -1) {
          return trimmed.slice(hashIndex + 2).replace(/[^a-z0-9-]/gi, '').toLowerCase();
        }
    ...
    const workspaceHub = DOM.workspaceHub ? new WorkspaceHub(DOM.workspaceHub, workspaceStore) : null;
